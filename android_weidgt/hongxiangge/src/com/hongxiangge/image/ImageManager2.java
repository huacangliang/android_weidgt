...
public class ImageManager2 {
	...
	public LruCache<String, Bitmap>			mMemoryCacheRound;
	...
	public static boolean					isTranFromUser		= true;

	private static HongxianggeApplication	myapp;

	/**
	 * 是否是左边侧滑头像
	 */
	public static boolean					isUserLeft_icon		= false;
	 ...
	 * @return
	 */
	public static ImageManager2 from(Context context, boolean isRound) {

		// 如果不在ui线程中，则抛出异常
		if (Looper.myLooper() != Looper
				.getMainLooper()) { throw new RuntimeException("Cannot instantiate outside UI thread."); }

		if (myapp == null) {
			myapp = (HongxianggeApplication) context.getApplicationContext();
		}
	...
	private ImageManager2(Context context) {
		...
		mMemoryCacheRound = new LruCache<String, Bitmap>(cacheSize) {
	...
	public void displayImage(ImageView imageView, String url, int resId, int width, int height) {
		...
		Bitmap bitmap = null;
		if (isRound) {
			bitmap = mMemoryCache.get(url + width + height);
		}
		else {
			bitmap = mMemoryCache.get(url + width + height);
		}
					...
					if (bitmap != null) {
						...
						if (imageRef.width != 0 && imageRef.height != 0) {
							...
							if (isRound) {
								...
								Bitmap bit = mMemoryCacheRound.get(url + imageRef.width + imageRef.height);
								if (bit == null) {
									bitmap = ImageManage.createFramedPhoto(imageRef.width, imageRef.height, bitmap, 360,
											context);
									mMemoryCacheRound.put(url + imageRef.width + imageRef.height, bitmap);
								}
								else {
									bitmap = bit;
								}
							...
							if (mMemoryCache.get(url) == null) mMemoryCache.put(url, bitmap);
						}
						...
						try {
							...
							if (data != null) {
								...
								int bitmapSize = opt.outHeight * opt.outWidth * 4;// pixels*3
																					...
																					// it's
																					// ARGB
								if (bitmapSize > 1000 * 1200) opt.inSampleSize = 2;
								opt.inJustDecodeBounds = false;
								...
								if (bitmap != null && url != null) {
									...
									if (imageRef.width != 0 && imageRef.height != 0) {
										...
										if (isRound) {
											Bitmap bit = mMemoryCacheRound.get(url + imageRef.width + imageRef.height);
											if (bit == null) {
												bitmap = ImageManage.createFramedPhoto(imageRef.width, imageRef.height,
														bitmap, 360, context);
												mMemoryCacheRound.put(url + imageRef.width + imageRef.height, bitmap);
											}
											else {
		...
		public void handleMessage(Message msg) {
			if (msg != null) {
				...
				case MSG_REPLY: // 收到应答
					...
					do {
						if (setOndwonListenear != null) {
							setOndwonListenear.setStateListenear(true);
						}
						...
						if (setOnDwonImage != null) {
							setOnDwonImage.setOnDwonBitMap(bitmap, imageRef.url);
						}
	...
	private void setImageBitmap(ImageView imageView, Bitmap bitmap, boolean isTran) {
		if (imageManager.isRound) {
			isTranFromUser = true;
			bitmap = ImageManage.createFramedPhoto(100, 100, bitmap, 360, context);
		}
		if (isTran && isTranFromUser) {
			TransitionDrawable td = new TransitionDrawable(
					new Drawable[] { new ColorDrawable(android.R.color.transparent),
							new BitmapDrawable(imageManager.context.getResources(), bitmap) });
			...
			if (!isTranFromUser) {
				BitmapDrawable background = new BitmapDrawable(imageManager.context.getResources(), bitmap);
	...
	private OndwonImage setOnDwonImage;

	public interface OndwonImage {

		void setOnDwonBitMap(Bitmap bitmap, String key);
	}

	public void setSetOnDwonImage(OndwonImage setOnDwonImage) {
		this.setOnDwonImage = setOnDwonImage;
	}

	private OndwonListenear setOndwonListenear;

	public interface OndwonListenear {

		void setStateListenear(boolean flag);
	}
	...
	public void setSetOndwonListenear(OndwonListenear setOndwonListenear) {
		this.setOndwonListenear = setOndwonListenear;
	}
