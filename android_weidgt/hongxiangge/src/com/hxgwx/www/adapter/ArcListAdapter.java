...
public class ArcListAdapter extends BaseAdapter {
	...
	private ArcTypeListActivity activity;
	...
	public void fillImage(String key, ImageView iv) {
		...
		if (key != null && key.length() > 0) {
			...
			if (iv.getTag().toString().trim().length() > 0
					&& iv.getTag().toString().equalsIgnoreCase(key)) {
				key = activity.getApplication2().getDomain() + key;
				if (!scoll) {
	...
	public void setOnlistener() {
		if (activity != null && activity.getPlv_arclist() != null) {
			activity.getPlv_arclist().setOnScrollListener(
			...
			activity.getPlv_arclist().setOnTouchListener(new OnTouchListener() {
	...
	public ArcListAdapter(Context context, List<ArticModelBase> baseList) {
		// TODO Auto-generated constructor stub
		this.context = context;
		if (context instanceof ArcTypeListActivity) {
			activity = (ArcTypeListActivity) context;
		}
