...
public class SearchListAdapter extends BaseAdapter {
	...
	public void fillImage(String key, ImageView iv) {
		...
		if (key != null && key.length() > 0) {
			...
			if (iv.getTag().toString().trim().length() > 0
					&& iv.getTag().toString().equalsIgnoreCase(key)) {
				key = fragment.getApplication2().getDomain() + key;
				if (!scoll) {
	...
	public void setOnlistener() {
		if (fragment != null && fragment.getLv_search_mian() != null) {
			fragment.getLv_search_mian().setOnScrollListener(
			...
			fragment.getLv_search_mian().setOnTouchListener(
					new OnTouchListener() {
	...
	public SearchListAdapter(Context context, SearchActivity searchFragment,
