...
public class ReplyActivity extends Activity implements ActivityManagerUtils {
	@Override
	...
	class Veriable {
		public Veriable() {
			...
			try {
				Utils.injectObject(this, ReplyActivity.this, R.id.class);
			} catch (Exception e) {
		...
		vid.et_speak.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!HongxianggeApplication.getInstance().isLogin) {
					...
					Toast.makeText(ReplyActivity.this, "请登录", 1000).show();
					return;
				}
				Intent intent = new Intent(ReplyActivity.this,
	...
	class Query extends Asyntask2<Boolean, String, Boolean> {
		...
		protected Boolean doInbackProgres(Boolean... params) {
			// TODO Auto-generated method stub
			FeedbackSubBase base = web.queryReply(aid, page, size);
			if (base != null) {
				if (base.isStatu()) {
					if (params[0]) {
						data.clear();
